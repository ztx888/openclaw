...
它可以通过你已经在使用的渠道回复你（WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat），还支持通过扩展接入 BlueBubbles, Matrix, Zalo, 和 Zalo Personal。它能够在 macOS/iOS/Android 上进行听说交互，并能渲染一个完全由你控制的实时 Canvas（画布）。Gateway（网关）只是控制平面 — 这个助手本身才是产品。

如果你想要一个感觉完全本地化、响应迅速且永远在线的个人单用户助手，这就是你的选择。
...
首选设置方式：运行初始化向导 (`openclaw onboard`)。它会引导你配置 Gateway、工作区、渠道和技能。CLI 向导是推荐的路径，支持 **macOS, Linux, 和 Windows (via WSL2; 强烈推荐)**。
支持 npm, pnpm, 或 bun。
新用户安装？请从这里开始：[快速开始](https://docs.openclaw.ai/start/getting-started)
...
模型说明：虽然支持任何模型，但我强烈推荐 **Anthropic Pro/Max (100/200) + Opus 4.5**，因为它具有长上下文优势和更好的抗提示注入能力。参见 [初始化配置](https://docs.openclaw.ai/start/onboarding)。

## 模型 (选择 + 认证)

- 模型配置 + CLI: [模型文档](https://docs.openclaw.ai/concepts/models)
- 认证配置轮换 (OAuth vs API keys) + 故障转移: [模型故障转移](https://docs.openclaw.ai/concepts/model-failover)
...
向导会安装 Gateway 守护进程（launchd/systemd 用户服务），确保持续运行。
...
# 与助手对话 (可选：投递回任何已连接的渠道：WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
...
- **stable**: 稳定版发布 (`vYYYY.M.D` 或 `vYYYY.M.D-<patch>`), npm dist-tag `latest`。
- **beta**: 预发布版本 (`vYYYY.M.D-beta.N`), npm dist-tag `beta` (macOS 应用可能缺失)。
- **dev**: `main` 分支的最新构建, npm dist-tag `dev` (发布时)。
...
注意: `pnpm openclaw ...` 会直接运行 TypeScript (通过 `tsx`)。`pnpm build` 会生成用于 Node 运行或打包 `openclaw` 二进制文件的 `dist/` 目录。

## 安全默认值 (私信访问)

OpenClaw 连接真实的通信界面。请将收到的私信 (DM) 视为 **不可信输入**。
...
- 批准方式: `openclaw pairing approve <channel> <code>` (发送者会被加入本地白名单)。
- 公共私信需显式开启: 设置 `dmPolicy="open"` 并在渠道白名单中包含 `"*"` (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`)。

运行 `openclaw doctor` 可检查有风险/配置错误的私信策略。
...
- **[多智能体路由](https://docs.openclaw.ai/gateway/configuration)** — 将入站渠道/账户/对等点路由到隔离的智能体（工作区 + 每智能体会话）。
- **[语音唤醒 (Voice Wake)](https://docs.openclaw.ai/nodes/voicewake) + [对话模式 (Talk Mode)](https://docs.openclaw.ai/nodes/talk)** — 基于 ElevenLabs 的 macOS/iOS/Android 全天候语音支持。
- **[实时 Canvas](https://docs.openclaw.ai/platforms/mac/canvas)** — 基于 [A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui) 的智能体驱动可视化工作区。
- **[一等公民工具](https://docs.openclaw.ai/tools)** — 浏览器、Canvas、节点、Cron、会话以及 Discord/Slack 操作。
- **[配套应用](https://docs.openclaw.ai/platforms/macos)** — macOS 菜单栏应用 + iOS/Android [节点](https://docs.openclaw.ai/nodes)。
- **[初始化向导](https://docs.openclaw.ai/start/wizard) + [技能](https://docs.openclaw.ai/tools/skills)** — 向导驱动的设置流程，包含捆绑/托管/工作区技能。
...
- [CLI 界面](https://docs.openclaw.ai/tools/agent-send)：gateway, agent, send, [向导](https://docs.openclaw.ai/start/wizard), 以及 [doctor](https://docs.openclaw.ai/gateway/doctor)。
- [Pi 智能体运行时](https://docs.openclaw.ai/concepts/agent)：支持 RPC 模式、工具流式传输和块流式传输。
- [会话模型](https://docs.openclaw.ai/concepts/session)：`main` (直接聊天)、群组隔离、激活模式、队列模式、回复机制。群组规则：[Groups](https://docs.openclaw.ai/concepts/groups)。
...
- [群组路由](https://docs.openclaw.ai/concepts/group-messages)：提及 (mention) 门控、回复标签、分渠道分块和路由。渠道规则：[Channels](https://docs.openclaw.ai/channels)。
...
- [节点 (Nodes)](https://docs.openclaw.ai/nodes)：相机拍照/剪辑、屏幕录制、[location.get](https://docs.openclaw.ai/nodes/location-command)、通知。
...
- [在线状态 (Presence)](https://docs.openclaw.ai/concepts/presence)、[输入指示器](https://docs.openclaw.ai/concepts/typing-indicators) 和 [使用量追踪](https://docs.openclaw.ai/concepts/usage-tracking)。
...
- **[Gateway WebSocket 网络](https://docs.openclaw.ai/concepts/architecture)** — 客户端、工具和事件的单一 WS 控制平面（加上运维文档：[Gateway runbook](https://docs.openclaw.ai/gateway)）。
- **[Tailscale 暴露](https://docs.openclaw.ai/gateway/tailscale)** — 用于 Gateway 仪表盘 + WS 的 Serve/Funnel（远程访问：[Remote](https://docs.openclaw.ai/gateway/remote)）。
- **[浏览器控制](https://docs.openclaw.ai/tools/browser)** — openclaw 管理的 Chrome/Chromium，支持 CDP 控制。
- **[Canvas + A2UI](https://docs.openclaw.ai/platforms/mac/canvas)** — 智能体驱动的可视化工作区（A2UI 主机：[Canvas/A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)）。
- **[语音唤醒 (Voice Wake)](https://docs.openclaw.ai/nodes/voicewake) + [对话模式 (Talk Mode)](https://docs.openclaw.ai/nodes/talk)** — 始终在线的语音和连续对话。
- **[节点 (Nodes)](https://docs.openclaw.ai/nodes)** — Canvas、相机拍照/剪辑、屏幕录制、`location.get`、通知，加上 macOS 专用的 `system.run`/`system.notify`。

## Tailscale 访问 (Gateway 仪表盘)

OpenClaw 可以自动配置 Tailscale **Serve** (仅 tailnet) 或 **Funnel** (公开)，且 Gateway 保持绑定到回环地址。配置 `gateway.tailscale.mode`:
...
完全可以在小型 Linux 实例上运行 Gateway。客户端（macOS 应用、CLI、WebChat）可以通过 **Tailscale Serve/Funnel** 或 **SSH 隧道** 连接，你仍然可以配对设备节点（macOS/iOS/Android）以在需要时执行设备本地操作。
...
macOS 应用可以以 **节点模式** 运行，并通过 Gateway WebSocket 广播其能力 + 权限映射 (`node.list` / `node.describe`)。客户端随后可以通过 `node.invoke` 执行本地操作：

- `system.run` 运行本地命令并返回 stdout/stderr/退出码；设置 `needsScreenRecording: true` 以请求屏幕录制权限（否则你会收到 `PERMISSION_MISSING`）。
- `system.notify` 发布用户通知，如果通知被拒绝则失败。
- `canvas.*`, `camera.*`, `screen.record`, 和 `location.get` 也可以通过 `node.invoke` 路由，并遵循 TCC 权限状态。

提升的 bash (主机权限) 与 macOS TCC 是分开的：

- 使用 `/elevated on|off` 在启用且在白名单中时切换每会话的提升访问权限。
- Gateway 通过 `sessions.patch` (WS 方法) 持久化每会话的切换状态，与 `thinkingLevel`, `verboseLevel`, `model`, `sendPolicy`, 和 `groupActivation` 一样。
...
## 智能体对智能体 (sessions_* 工具)

- 使用这些工具在不同会话之间协调工作，无需在聊天界面之间跳转。
- `sessions_list` — 发现活跃会话（智能体）及其元数据。
- `sessions_history` — 获取会话的转录日志。
- `sessions_send` — 给另一个会话发送消息；可选回复 ping-pong + 公告步骤 (`REPLY_SKIP`, `ANNOUNCE_SKIP`)。
...
- `/status` — 紧凑的会话状态 (模型 + tokens，如有成本则显示)
- `/new` 或 `/reset` — 重置会话
- `/compact` — 压缩会话上下文 (摘要)
...
极简 `~/.openclaw/openclaw.json` (模型 + 默认值):
...
- **默认:** 工具在 **main** 会话的主机上运行，因此在你独占使用时，智能体拥有完全访问权限。
- **群组/渠道安全:** 设置 `agents.defaults.sandbox.mode: "non-main"` 以在每会话 Docker 沙盒中运行 **非 main 会话**（群组/渠道）；此时 bash 在这些会话的 Docker 中运行。
- **沙盒默认值:** 允许列表包括 `bash`, `process`, `read`, `write`, `edit`, `sessions_list`, `sessions_history`, `sessions_send`, `sessions_spawn`; 拒绝列表包括 `browser`, `canvas`, `nodes`, `cron`, `discord`, `gateway`。
...
- 可选: 设置 `channels.telegram.groups` (配合 `channels.telegram.groups."*".requireMention`); 设置后即为群组白名单 (包含 `"*"` 以允许所有)。按需设置 `channels.telegram.allowFrom` 或 `channels.telegram.webhookUrl` + `channels.telegram.webhookSecret`。
...
- 通过 `msteams.allowFrom` 设置谁可以对话；群组访问通过 `msteams.groupAllowFrom` 或 `msteams.groupPolicy: "open"`。
...
当你是通过初始化流程后，想要查阅更深入的参考资料时使用。

- [从文档索引开始导航和了解“什么在哪”。](https://docs.openclaw.ai)
- [阅读架构概览以了解 gateway + 协议模型。](https://docs.openclaw.ai/concepts/architecture)
- [当需要每个键和示例时，使用完整的配置参考。](https://docs.openclaw.ai/gateway/configuration)
- [参考运维手册按标准运行 Gateway。](https://docs.openclaw.ai/gateway)
...
- [按照初始化向导流程进行引导式设置。](https://docs.openclaw.ai/start/wizard)
...
- [TypeBox 图式](https://docs.openclaw.ai/concepts/typebox)
...
特别感谢 [Mario Zechner](https://mariozechner.at/) 的支持以及他的 [pi-mono](https://github.com/badlogic/pi-mono) 项目。
